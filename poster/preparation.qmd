...
# Data Cleaning
## Define Disaster Sub-Categories
```{r}
...
# Get cost of BDD events by year, and add disaster subcategory
bdd_cost <- bdd_time_series |>
  ...
  mutate(Category = disaster_mapping[Disaster])
bdd_cost
...
# Add disaster subcategory to the additional events
bdd_additional_events <- bdd_additional_events |> 
  mutate(Category = case_when(
    Disaster %in% names(disaster_mapping) ~ disaster_mapping[Disaster],
    TRUE ~ "Other"
...
# Prepare subset of additional information to be added to the base events tibble
