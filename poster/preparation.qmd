...
Get frequency and costs of BDD events by year, which is equivalent to the `bdd_events` and `bdd_frequency` tibbles, but not including categories with values = `0`. This is to enable cross-checking and data verifications to be performed, to ensure that the values in `bdd_events` and `bdd_frequency`, derived from the time series dataset, are accuracte and correct.
```{r}
