...
## Data
-   Frequency and estimated cost for each disaster type is obtained from the National Centers for Environmental Information (NCEI). [^ncei_timeseries] 
-   Consolidated data with NCEI USA to obtain more detailed information on each disaster specific to the United States.[^ncei_usa]

[^ncei_timeseries]: <https://www.ncei.noaa.gov/access/billions/time-series>
[^ncei_usa] : <https://www.ncei.noaa.gov/access/billions/state-summary/US>
...
bdd_top_cost_summary <- bdd_top_cost_summary |>
  mutate(
    custom_vjust = c(-0.85, -0.85, 0.5, 0.5),
    custom_hjust = c(0.5, 0.5, 1, 1),
    custom_xnudge = c(0, 0, -0.3, -0.15)
  )
...
bdd_cost_plot <- ggplot(bdd_cost_combined) +
  ...
  geom_point(data = bdd_top_cost_summary, size = 1.8, color = "black") +
  labs(
    x = "Year",
