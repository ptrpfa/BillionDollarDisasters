...
# Define custom colors for each category, based on the Paired color palette from RColorBrewer
category_colors <- c("All Events" = "#6A3D9A",
...
bdd_cost_plot <- ggplot(bdd_cost_combined) +
  aes(x = Interval, y = Cost,  color = Category, group = Category, linetype = Category) +
  ...
  labs(
    ...
    color = "Disaster Type",
    title = "Frequency and Estimated Costs of Billion-Dollar Disasters in the USA (1980-2024)"
  ) +
  theme_minimal() +
  theme(
    ...
    axis.title.x = element_text(size = 14, hjust = 1.07, margin = margin(t = -13)),
  ...
  scale_linetype_manual(
    values = c("All Events" = "dashed", "Dry Weather Events" = "solid",
               "Storm Events" = "solid", "Wet Weather Events" = "solid")
  ) +
  scale_y_continuous(breaks = seq(0, max(bdd_cost_summary_aggregated$Cost) + 50, by = 50)) +
  guides(
    shape = "none",
    fill = "none",
    linetype = "none",
    color = guide_legend(override.aes = list(fill = c("#6A3D9A","#33A02C","#FF7F00", "#1F78B4"), color = NA, size = 3, alpha = 1))
  )
bdd_cost_plot
```
...
bdd_freq_plot <- ggplot() +
  geom_bar(data = bdd_frequency_summary, aes(x = Interval, y = Count, fill = Category, group = Category), stat = "identity", width = 0.55) +
  geom_smooth(data = bdd_frequency_total, aes(x = Interval, y = TotalCount, group = 1), color = "#6A3D9A", linetype = "dashed", size = 1.1, span = 0.7, se = FALSE) +
  labs(
    y = "Frequency",
...
#| fig-width: 10
#| fig-height: 10
#| fig-cap: "Revised Visualisation of Frequency and Estimated Costs of Billion-Dollar Disasters in the USA by year."
#| warning: false
