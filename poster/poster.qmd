...
# Faceted line graph
bdd_cost_line_plot <- bdd_cost_summary |>
  ggplot(aes(x = Interval, y = Cost, color = Category, fill = Category, group = Category)) +
  geom_line() +
  geom_point() +
  ...
  scale_y_continuous(breaks = seq(0, max(bdd_cost_summary$Cost + 100), by = 100)) +
  guides(color = guide_legend(), fill = guide_legend())  # Ensure only one legend is displayed
bdd_cost_line_plot

bdd_cost_summary$Category <- factor(bdd_cost_summary$Category , levels=c("Storm Events", "Dry Weather Events", "Wet Weather Events"))

# Stacked area plot
bdd_cost_plot <- bdd_cost_summary |>
  ggplot(aes(x = Interval, y = Cost, fill = Category, group = Category)) +
  geom_area(position = "stack", alpha = 0.6) +
  labs(
    x = "Year",
    y = "Cost (CPI-Adjusted, Billion $USD)",
